        ...
        if item_decl["kind"] == "EnumConstantDecl":
            ...
            if "inner" in item_decl:
                ...
                if const_expr["kind"] != "ConstantExpr":
                    sys.exit(
                        f"ERROR: Enum values must be a ConstantExpr ({item_decl['name']}), is '{const_expr['kind']}'"
                    )
                if (
                    const_expr["valueCategory"] != "rvalue"
                    and const_expr["valueCategory"] != "prvalue"
                ):
                    sys.exit(
                        f"ERROR: Enum value ConstantExpr must be 'rvalue' or 'prvalue' ({item_decl['name']}), is '{const_expr['valueCategory']}'"
                    )
                if not (
                    (len(const_expr["inner"]) == 1)
                    and (const_expr["inner"][0]["kind"] == "IntegerLiteral")
                ):
                    sys.exit(
                        f"ERROR: Enum value ConstantExpr must have exactly one IntegerLiteral ({item_decl['name']})"
                    )
    ...
    if "inner" in decl:
        for param in decl["inner"]:
            if param["kind"] != "ParmVarDecl":
                print(
                    f"  >> warning: ignoring func {decl['name']} (unsupported parameter type)"
                )
                return None
            outp_param = {}
...
def clang(csrc_path):
    cmd = [
        r"C:\Users\phcre\Downloads\clang-18.1.8-windows-amd64-msvc17-libcmt\bin\clang.exe",
        "-Xclang",
        "-ast-dump=json",
        "-c",
    ]
